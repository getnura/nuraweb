...
class PageController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        if ($parent_slug) {
            $parent_content = Page::where('slug', $parent_slug)->where('active', 1)->first();
            if (!$parent_content) return redirect(route('home'));
        }

        if (!$page->meta_title) $page->meta_title = $page->title;
        if (!$page->meta_description) $page->meta_description = $page->meta_title ?? $page->title;
        if ($page->layout_id) $layout = TemplateLayout::find($page->layout_id);

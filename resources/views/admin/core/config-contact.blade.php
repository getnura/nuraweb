        ...
        <form method="post">
            ...
            <div class="form-group mb-0">
                <div class="form-check form-switch">
                    <input type='hidden' value='' name='contact_page_disabled'>
                    <input class="form-check-input" type="checkbox" id="contact_page_disabled" name="contact_page_disabled" @if ($config->contact_page_disabled ?? null) checked @endif>
                    <label class="form-check-label" for="contact_page_disabled">{{ __('Disable contact page') }}</label>
                </div>
            </div>

            @if ($config->contact_page_disabled ?? null)
                <div class="fw-bold text-danger">{{ __('Contact page is disabled.') }}</div>
            @else
                <div class="fw-bold text-success">{{ __('Contact page is enabled.') }}</div>
            @endif

            <script>
                $('#contact_page_disabled').change(function() {
                    select = $(this).prop('checked');
                    if (select) {
                        document.getElementById('hidden_div_contact_page').style.display = 'none';
                    } else {
                        document.getElementById('hidden_div_contact_page').style.display = 'block';
                    }
                })
            </script>

            <div id="hidden_div_contact_page" style="display: @if ($config->contact_page_disabled ?? null) none @else block @endif" class="mt-3">

...
@if ($block_data->content ?? null)
    @php
        $item = unserialize($block_data->content->content);
    @endphp
        ...
        <blockquote @if ($block_extra['shaddow'] ?? null) class="img-shaddow" @endif>
            ...
            @if ($item['source'] ?? null)
                <div class="block-blockquote-source">
                    @if (($block_extra['use_avatar'] ?? null) && ($block_extra['avatar'] ?? null))
                        <img class="block-blockquote-avatar img-fluid" alt="{{ $item['source'] }}" src="{{ image($block_extra['avatar']) }}">
                    @endif
                    {!! $item['source'] !!}
                </div>
            @endif
